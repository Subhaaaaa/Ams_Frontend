...
namespace AMS_API.DataAccess
{
    public class RequestDataAccess
    {
        ...
        public Request GetRequest(int requestId)
        {
            ...
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                try
                {
                    ...
                    string sqlQuery = @"SELECT Distinct TR.RequestId, TA.AssetId, TA.AssetName, TAC.AssetCategoryId, TAC.AssetCategory, TU.UserId, TU.Name, TR.CreatedOn, TR.IsResolved
        ...
        public int InsertAsset(Asset assets)
        {
            int noOfRowsInserted = 0;
            int result;
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                try
                {
                    string sqlQuery = @"Insert into tbl_Assets(AssetTypeId,SerialNoOrLicenseNo,BrandId,ManufacturingDate,ExpireDate,IsWorking,IsWired) values(@AssetTypeId,@SerialNo,@BrandId,@ManufacturingDate,@ExpireDate,@IsWorking,@IsWired)";
                    using (SqlCommand cmd = new SqlCommand(sqlQuery, conn))
                    {
                        cmd.CommandType = CommandType.Text;
                        cmd.CommandText = sqlQuery;
                        cmd.Parameters.AddWithValue("@AssetTypeId",assets.AssetType.AssetTypeId);
                        cmd.Parameters.AddWithValue("@SerialNo",assets.SerialNo);
                        cmd.Parameters.AddWithValue("@brandId",assets.Brand.BrandId);
                        cmd.Parameters.AddWithValue("@ManufacturingDate",assets.ManufacturingDate);
                        cmd.Parameters.AddWithValue("@ExpireDate", assets.ExpireDate);
                        cmd.Parameters.AddWithValue("@IsWorking", assets.IsWorking);
                        cmd.Parameters.AddWithValue("IsWired",assets.IsWired); 
                    ...
                    if (result != 0) ;
                    {
                        var requestStatus=new RequestStatus();
                        requestStatus.Request.RequestId = result;
                        requestStatus.Status.StatusId = result;
                        noOfRowsInserted = InsertRequestStatus(requestStatus);
        ...
        public IList<Asset> GetAsset()
        {
            ...
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                try
                {
                    ...
                    SqlCommand cmd = conn.CreateCommand();
                    string sqlQuery = @"select ta.AssetId,tat.AssetTypeName,ta.SerialNoOrLicenseNo,tac.AssetCategory,tb.BrandName,ta.ManufacturingDate,
                                            ta.ExpireDate,ta.IsWired,ta.IsWorking,ta.CreatedOn
                                            from tbl_Assets ta join tbl_AssetType tat on ta.AssetTypeId=tat.AssetCategoryId
                                            join tbl_AssetCategory tac on tac.AssetCategoryId=tat.AssetCategoryId 
		                                    join tbl_Brands tb on tb.BrandId=ta.BrandId;";
                    cmd.CommandText = sqlQuery;
                    ...
                    if (assetDt.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in assetDt.Rows)
                        {
                            ...
                            asset.AssetType.AssetTypeName=(row[1]).ToString();
                            asset.SerialNo= (row[2]).ToString();
                            asset.AssetCategory.AssetCategoryName=(row[3]).ToString();
                            asset.Brand.BrandName=(row[4]).ToString();  
                            asset.ManufacturingDate= Convert.ToDateTime(row[5]);
                            asset.ExpireDate= Convert.ToDateTime(row[6]);
                            asset.IsWorking= Convert.ToBoolean(row[7]);
                            asset.IsWired= Convert.ToBoolean(row[8]);
                            asset.CreatedOn= Convert.ToDateTime(row[9]);
        ...
        public IList<Brand> GetBrands()
        {
            ...
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                try
                {
                    brandsDt = new DataTable();
                    SqlCommand cmd = conn.CreateCommand();
                    string sqlQuery = @"SELECT * FROM tbl_Brands";
                    cmd.CommandText = sqlQuery;
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);

                    adapter.Fill(brandsDt);

                    if (brandsDt.Rows.Count > 0)
                    {
                        brands = new List<Brand>();
                        foreach (DataRow row in brandsDt.Rows)
                        {
                            var brand = new Brand();
                            brand.BrandId = Convert.ToInt32(row[0]);
                            brand.BrandName = (row[1]).ToString();
